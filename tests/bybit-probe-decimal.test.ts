...
test("decimal arithmetic normalizes strings without binary floating point", () => {
  ...
  assert.equal(toDecimalString(multiplyDecimals(parseDecimal("1.25"), parseDecimal("2.4"))), "3");
  assert.equal(compareDecimals(parseDecimal("10.00"), parseDecimal("10")), 0);
});

test("step rounding is directional at exact boundaries", () => {
  assert.equal(toDecimalString(ceilToStep(parseDecimal("1.001"), parseDecimal("0.01"))), "1.01");
  assert.equal(toDecimalString(floorToStep(parseDecimal("1.019"), parseDecimal("0.01"))), "1.01");
  assert.equal(toDecimalString(ceilToStep(parseDecimal("5.06"), parseDecimal("0.01"))), "5.06");
});

test("rational price factors retain exact decimal results", () => {
  assert.equal(toDecimalString(multiplyByRational(parseDecimal("100.00"), 102n, 100n)), "102");
  assert.equal(toDecimalString(multiplyByRational(parseDecimal("100.00"), 98n, 100n)), "98");
});
