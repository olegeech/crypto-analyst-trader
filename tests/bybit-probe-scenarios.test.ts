...
test("reconciliation treats acknowledgement as pending and delayed visibility as non-submission", async () => {
  ...
  const transport: ScenarioTransport = {
    async get(path) {
      ...
      lookups += 1;
      return response({
        list: lookups === 1 ? [] : [{ orderId: "exchange-1", orderLinkId: "run-1-long", orderStatus: "New", takeProfit: "0.102", stopLoss: "0.098" }],
      });
    },
    async post() {
...
test("history fallback resolves only after realtime budget is exhausted", async () => {
  ...
  const transport: ScenarioTransport = {
    async get(path) {
      calls.push(path);
      return response({
        list: path === "/v5/order/history" ? [{ orderId: "exchange-2", orderLinkId: "run-1-long", orderStatus: "Cancelled" }] : [],
      });
    },
    async post() {
      throw new Error("not used");
...
test("entry dispatch persists the exact approved plan before POST and records attached-exit evidence", async () => {
  ...
  try {
    const store = new ProbeStore({ rootDir: root, clock: () => 1_000, processId: 101, isProcessAlive: () => false });
    const calls: string[] = [];
    const transport: ScenarioTransport = {
      async get() {
        return response({ list: [{ orderId: "exchange-1", orderLinkId: "run-1-long", orderStatus: "New", takeProfit: "0.102", stopLoss: "0.098" }] });
      },
    ...
    const result = await runEntryScenario({
      ...
      clock: () => 1_000,
      sleep: async () => undefined,
      approve: async () => ({ kind: "approved", plan, digest: hashProbePlan(plan), approvedAt: 1_000, expiresAt: plan.expiresAt }),
      realtimeAttempts: 2,
      historyAttempts: 0,
    });
